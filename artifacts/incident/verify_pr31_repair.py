...
# ------------------------------------------------------------- G1 necessity
# NECESSITY is now proven against a tree with the repair DELIBERATELY REVERTED,
# because post-merge `main` is (correctly) no longer divergent. Reverting in a
# throwaway copy is the only honest way to keep witnessing that the fix is
...
for td in (td0, td1, td2, td3):
    shutil.rmtree(td, ignore_errors=True)

passed = sum(1 for _, ok, _ in RESULTS if ok)
print("\nGATES: " + str(passed) + "/" + str(len(RESULTS)) + " passed")
if passed == len(RESULTS):
    print("\nVERDICT: the INC-31 repair is PRESENT, NECESSARY, SUFFICIENT, and NOT a")
    print("         cover-up. Strip the scrub and this verifier reddens.")
    sys.exit(0)
