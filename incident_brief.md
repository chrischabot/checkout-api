...
> ### ⚠️ Read this before any incident claim below
> **The three production telemetry sources yielded ZERO payloads: 0 Sentry issues, 0 OTEL traces, 0 gateway log lines.** The pull was *attempted* against all three and is **fully instrumented** — but the data is not there to pull:
> - **Sentry** — the **credential is absent** (egress works: `/api/0/` → 200; `/organizations/` → **401**)
> - **OTEL** — **no collector is running** (11 standard ports probed, 0 open)
> - **Gateway logs** — **no log source exists** on disk or mounted (11 paths checked)
>
> **So the telemetry-dependent half of this run could not be completed, and is not claimed as complete.** No symptom clustering across Sentry/OTEL/logs happened, because there were no symptoms to cluster. **Blast radius is UNKNOWN and deliberately NOT estimated** — any number here would be fabricated.
>
> What *was* available: **GitHub** (32 PRs across 3/3 fleet repos, 1 deployment record) and **the deployed source itself**. Every defect below was therefore established by **executing production code**, which is why the findings are still real, live, and reproducible — just not *bounded*.
>
> **The single highest-value fix to this loop: wire a Sentry credential.** Until then every run is blind to customer impact.
...
| **OPEN, needs a one-line rebase** — INC-33 probe hardening (**PR #36**) | `checkout-api` | ⚠️ **see §6.1** |
| **CLOSED** — #34, #35 (same tooling conflict, superseded by #36) | `checkout-api` | ✅ consolidated |
...
### 6.1 One thing I could NOT finish — with the cause PROVEN, not guessed

**PR #36 (INC-33, probe hardening) is open and reports `mergeable: false / dirty`.** The change is verified and green, but it cannot be landed with the tools available to me.

**The cause is measured, not assumed.** `git compare main...fabric/inc-33-final` returns:

```
status: diverged    ahead_by: 1    behind_by: 2
```

The push tooling rebuilds every branch from a **registered baseline commit two behind `main`**, so it re-applies content that has already merged — `artifacts/incident/probe_telemetry.py` even shows as **`added`** in the diff, though it exists on `main` today. That is why **three** attempts (#34, #35, #36) all conflicted regardless of how the files were staged. **A tooling constraint, not a defect in the change.**

**To land it:** the true delta against current `main` is **two files** — `artifacts/incident/probe_telemetry.py` and `incident_brief.md`. Rebase the branch, cherry-pick `75dd895`, or copy those two files onto a fresh branch cut from `main`.

> **What this does NOT block.** The **safe minimal patch this run existed to deliver is already merged.** Verified by AST against a fresh clone of `main`: **7 python spawns, 0 without `env=`, `child_env` defined in 6 files.** The env-var leak is **closed on `main`** (`29cd3f6`, plus guard + brief in `17135d2`). PR #36 is *only* the instrumentation hardening on top.

Saying "done" here would be the fleet's own disease — **a green claim the tree does not support.** So it is reported as a blocker, with the exact remedy.
