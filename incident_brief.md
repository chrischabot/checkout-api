...
| | |
|---|---|
| **Patched & verified this run** | **1** — INC-15: the fleet's cross-fleet re-confirmation gates were unreachable dead code reporting `6/6 passed`. **Merged to `main` as `5f70fae`** ([PR #14](https://github.com/chrischabot/checkout-api/pull/14), CI green) |
...
### INC-15 verifier — **9/9, exit 0** (commander workspace, siblings present)
...
| **G2b the shipped verifier still carries the REAL repo names** (static — **no siblings needed, so it runs in CI**) | `_TARGET_DIRS=('fabric-ic-incident-target','incident-target')` · `_GATEWAY_DIRS=('fabric-gateway-demo','gateway')` |
...
| G8 no production drift | every deployed source present matches its **full sha256** baseline |

*(`G0` fires only in strict mode where the siblings are required but missing — see the CI section below. G2b was added after the first draft of this brief, which is why the verifier now reports **9** executed gates in the commander workspace rather than 8.)*
...
- **New `G2b`** — a *static* assertion that the shipped verifier still carries the real fleet repo names. It needs **no siblings**, so it guards the repair **inside CI**. This is what keeps the fix from rotting: strip a real name and G2b goes **RED (4/5, exit 1)** — identically in CI and in the commander workspace, verified both ways.
...
**GitHub Actions on the final head (`9a85251`): all 8 steps green**, including the INC-15 step — [run 29172762790](https://github.com/chrischabot/checkout-api/actions/runs/29172762790). The gate does not merely pass locally; it passes *in the job that runs it*, and it still bites there.

**Merged to `main` as `5f70fae`** ([PR #14](https://github.com/chrischabot/checkout-api/pull/14)).

### Re-verified against MERGED `main` (fresh clone, exactly as CI sees it)

The claim that matters is not "the PR was green" but "the repair is live on `main`". Fresh clone of merged `main`, no siblings present:

| Check | Result |
|---|---|
| `npm test` | **10 tests, 10 pass / 0 fail** |
| `verify_inc9_ci_gate.py` | exit 0 — **`GATES: 6/6 passed (1 SKIPPED — NOT counted as passes)`** |
| `verify_inc12_ci_runs_verifier.py` | exit 0 |
| `verify_inc15_cross_fleet_discovery.py` | exit 0 — **`5/5 passed (4 SKIPPED — NOT counted as passes)`** |
| `ci.yml` steps on `main` | `['npm test', 'verify_inc9_ci_gate.py', 'verify_inc12_ci_runs_verifier.py', 'verify_inc15_cross_fleet_discovery.py']` |

**The `verify_inc9_ci_gate.py` row is the whole repair, visible on `main`:** the un-run cross-fleet gate is now **reported as `SKIPPED` and excluded from the tally** — `GATES: 6/6 passed (1 SKIPPED — NOT counted as passes)` — where before the merge the identical situation was silently folded into a confident `6/6 passed` while G6a/G6b/G6c were unreachable dead code. Same numerator; the difference is that the un-run gate is now *declared* instead of hidden.

### The gates provably bite (mutation test on the SHIPPED code)

A gate made *green* rather than *correct* is worthless, so the repaired discovery was regressed back to the blind names **inside the shipped verifier**, and the result re-measured against the current 9-gate verifier:

| Tree | INC-15 result |
|---|---|
| repaired, commander workspace (as shipped) | **9/9, exit 0** |
| **shipped names regressed to the blind names** (workspace, siblings present) | **4/5 passed, 4 SKIPPED — `G2b` FAILS, exit 1** ✅ caught |
| **shipped names regressed, on a BARE checkout** (CI conditions, no siblings) | **4/5 passed — `G2b` FAILS, exit 1** ✅ caught |
| restored | **9/9, exit 0** |

**Read the middle rows carefully — they are the design paying off.** Regressing the names makes discovery blind again, so the sibling-dependent gates (G1/G2/G4/G5) can no longer execute and *correctly* report `SKIPPED`. If `G2b` did not exist, the verifier would then report a vacuous "all executed gates passed" — **the very laundering bug INC-15 is about, resurrected one level up.** `G2b` is the gate that actually bites: it is static, needs no siblings, and **fails identically in the commander workspace and in CI**. That is why it was added.

This also closes a real review finding: the gates drive the **deployed** lookup function (`_find` / `_TARGET_DIRS` / `_GATEWAY_DIRS` imported from the shipped verifier), not a private copy of the fix — so a regression in the shipped discovery cannot pass unnoticed.
...
|---|---|
| `checkout-api` — `npm test` | **10 tests, 10 pass / 0 fail** |
| `checkout-api` — INC-9 verifier (strict, workspace) | **9/9, exit 0** — G6a/G6b/G6c executing |
| `checkout-api` — INC-12 verifier | **6/6, exit 0** |
| `checkout-api` — **INC-15 verifier (new)** | **9/9, exit 0** |
...
**36 tests + 7 verifiers, zero failures**, with non-vacuous counts (no "0 tests ran"). All three production sources hash-match their deployed revisions.
